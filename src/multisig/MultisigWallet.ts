...
export class MultisigWallet {
    ...
    static async fromAddress(address: Address, opts: { provider?: ContractProvider, client?: TonClient }): Promise<MultisigWallet> {
    ...
    public async sendOrder(order: MultisigOrder, secretKey: Buffer, provider?: ContractProvider) {
        if (!provider && !this.provider) {
            throw('you must specify provider if there is no such property in MultisigWallet instance');
        }
        if (!provider) {
